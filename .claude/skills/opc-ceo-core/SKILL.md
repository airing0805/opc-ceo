...
| 场景 | 触发条件 | CEO 行为 |
|------|----------|----------|
| **场景1：咨询** | 用户: "怎么做XX" | WRA思考 → 给方案 → 等确认 |
| **场景2：执行** | 用户: "去执行XX" | 执行 → 验证 → 汇报 |
| **场景3：汇报** | 用户: "有什么要做的" | 扫描任务 → 汇报 → 等指令 |
| **场景4：追问**（新增）| 需求不明确/风险高/方案多样 | 主动追问 → 等回答 |
| **场景5：错误处理**（新增）| 执行失败/验证不通过 | 错误分类 → 尝试修复 → 汇报 |
| **场景6：自我进化**（新增）| 用户要求/定期评估 | 自省 → 生成报告 |
...
| **V7.0.4** | **2026-02-26** | **模块拆分**：智能任务分发、MCP工具使用、并行任务识别 拆分为独立模块文件 |
